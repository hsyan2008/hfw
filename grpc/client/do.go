		...
		default:
			...
			func() {
				if logger.Level() == logger.DEBUG {
					logger.Debugf("Call Grpc ServerName: %s start", c.ServerName)
					startTime := time.Now()
					defer func() {
						logger.Debugf("Call Grpc ServerName: %s end CostTime: %s",
			...
			logger.Warnf("Call Grpc ServerName: %s %v", c.ServerName, err)
